    ...
    const load = async () => {
      ...
      try {
        const url =
          category === "Semua"
            ? `${BASE}/v1/channels`
            : `${BASE}/v1/channels?category=${category}`;
  ...
  const playDash = async (dashStream, video, showError) => {
    try {
      ...
      if (!shaka.Player.isBrowserSupported()) {
        showError("Browser tidak support DASH");
        return;
      }
      ...
      // inject API key ke semua request
      player
        .getNetworkingEngine()
